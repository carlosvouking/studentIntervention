...
\centering
    ...
    \label{SVMNonlinear2}}
    \caption{\textbf{Kernel trick.} \textit{Figure (a) shows data given by two features $\vec{x} = \{x_1,x_2\}$. Figure (b) shows the same data mapped to z by $\vec{x} = \{x_1,x_2\} \longrightarrow \vec{z} = \{x_1^2, \sqrt2x_1x_2, x_2^2 \}$. By mapping the data from two dimensions to three dimension, the data now becomes linearly separable in the new representation.}}
\end{figure}

The \textit{kernel} trick of SVMs make linear models work in nonlinear settings by mapping the data into higher dimension where we can see the linear behavior. For example, the figure in \ref{SVMNonlinear1} shows a features in two-dimensional space where it is evident we will not be able to find a decision boundary that separates the data into two classes. Using a kernel, the features are projected into a three dimension space as is shown in figure \ref{SVMNonlinear2} where we can find a hyperplane that will linearly separate the data into two classes.
