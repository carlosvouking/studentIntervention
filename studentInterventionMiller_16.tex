...
The theoretical time \& space complexity of decision trees classifiers as implemented in sci-kit learn package is $O(v_{features} n^2_{samples} log(n_{samples}))$.
...
What are its strengths and weaknesses?\\  
       ...
       \item Strengths of decision trees:
              \begin{itemize}[noitemsep,nolistsep]
                     ...
                     \item The cost of using the tree (i.e., predicting data) is logarithmic in the number of data points used to train the tree. In average case the cost of training $ O(pNlog^2N)$
...
$O(v_{features} * n^2_{samples} log(n_{samples}))$. For a random forest with mtry as the number of features sampled at each node, the complexity would be $O( n_{estimators} * mtry * * n^2_{samples} log(n_{samples}) )$
...
\setlength{\extrarowheight}{1.5pt}
\begin{table}[!htbp]
\caption{Random Forest Classifier - Confusion Matrix} %title of the table
\centering % centering table
\begin{tabular}{ |l|l|p{3cm}|p{3cm}| }
\hline % inserts single-line
\multirow{2}{*}{} & & \multicolumn{2}{c|}{Actual Class} \\ 
...
\begin{itemize}
\item What is the theoretical $O(n)$ time \& space complexity in terms of input size?\\
The theoretical time \& space complexity of SVMs scales between $O(n_{features} * n^2_{samples})$ and $O(n_{features} * n^3_{samples})$. Its a costly algorithm since the compute and storage requirements increase rapidly with the number of training vectors.
...
\item Given what you know about the data so far, why did you choose this model to apply?\\
The general approach is to try a few different algorithms for each problem. Sci-kit learn provides a guide to choosing estimators. Based on the guide, and satisfying the following criteria: {100K \textgreater samples \textgreater 50, Predicting a category \& Labeled data}, a linear SVC was selected. 
\end{itemize}


\setlength{\extrarowheight}{1.5pt}
\begin{table}[!htbp]
\caption{Result of training with a SVMs} %title of the table
\centering % centering table
\begin{tabular}{|p{6cm}|p{1.5cm}|p{1.5cm}|p{1.5cm}|} % creating four columns
\hline % inserts single-line
& \multicolumn{3}{c|}{Training set size}\\[5pt]
...
\setlength{\extrarowheight}{1.5pt}
\begin{table}[!htbp]
\caption{SVMs - Confusion Matrix} %title of the table
\centering % centering table
\begin{tabular}{ |l|l|p{3cm}|p{3cm}| }
\hline % inserts single-line
\multirow{2}{*}{} & & \multicolumn{2}{c|}{Actual Class} \\ 
